...
SELECT id_usuario, nombre, apellido, email, edad, roles.nombre_rol, categorias.nombre_categoria FROM usuarios INNER JOIN roles ON usuarios.id_rol = roles.id_rol INNER JOIN categorias ON usuarios.id_categoria = categorias.id_categoria;
...
-- Tu código aquí
ALTER TABLE usuarios ADD COLUMN id_categoria INT;
UPDATE usuarios SET id_categoria = 1 WHERE id_usuario BETWEEN 1 AND 5;
UPDATE usuarios SET id_categoria = 2 WHERE id_usuario BETWEEN 6 AND 10;
UPDATE usuarios SET id_categoria = 3 WHERE id_usuario BETWEEN 11 AND 15;
UPDATE usuarios SET id_categoria = 4 WHERE id_usuario BETWEEN 16 AND 20;
...
CREATE TABLE usuarios_categorias (id_usuario_categoria INT AUTO_INCREMENT PRIMARY KEY, id_usuario INT NOT NULL, id_categoria INT NOT NULL, FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario), FOREIGN KEY(id_categoria) REFERENCES categorias(id_categoria));
...
SELECT usuarios.id_usuario, usuarios.nombre, usuarios.email, usuarios.edad, roles.nombre_rol, categorias.nombre_categoria FROM usuarios
LEFT JOIN roles ON usuarios.id_rol = roles.id_rol 
LEFT JOIN usuarios_categorias ON usuarios_categorias.id_usuario = usuarios.id_usuario 
LEFT JOIN categorias ON categorias.id_categoria = usuarios_categorias.id_categoria